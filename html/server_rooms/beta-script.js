...
function onload(){
  ...
  socket.on("join", function(room){
    ...
    dingSound = document.getElementById("DingS");
    socket.emit("room", chatIDInput.value, usernameInput.value, "BETA");
  })
...
function Disconnect(){
  ...
  socket.emit("leave-room", chatIDInput.value, usernameInput.value, "BETA");
}
...
function Send(){
  if(messageInput.value.length < 401){
    if (delay && messageInput.value.replace(/\s/g, "") != ""){
      ...
      socket.emit("logmsg", chatIDInput.value, usernameInput.value, messageInput.value, "BETA");
      messageInput.value = "";
    }
    elif
  }
